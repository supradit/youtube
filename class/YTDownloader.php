...
class YTDownloader
{
    ...
    public function getDownloadLinks($id)
    {
        ...
        if (isset($videoData['url_encoded_fmt_stream_map']) && isset($videoData['adaptive_fmts'])) {
            ...
            foreach ($mLink as $linker) {
                if($linker['itag'] == 140 || $linker['itag'] == 18 || $linker['itag'] == 22 || $linker['itag'] == 137 || $linker['itag'] == 136){
                //if($linker['itag'] == 137){
                    if (isset($linker['s'])) {
    ...
    public function getItag($value){
        $itag_info = $this->itag_ext;
        return $itag_info[$value];
    }
}